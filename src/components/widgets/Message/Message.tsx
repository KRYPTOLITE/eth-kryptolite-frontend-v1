...
const MessageContainer: React.FC<
  MessageProps & { children: React.ReactNode }
> = ({ children, className }) => (
  <div
    className={cls("flex bg-red-50 p-4 rounded-2xl border gap-2", className)}
  >
    {children}
  </div>
);
...
export const MessageText: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
...
const Message: React.FC<MessageProps & { children: React.ReactNode }> = ({
  children,
  variant,
  icon,
  ...props
  ...
  return (
    <MessageContext.Provider value={{ variant }}>
      <MessageContainer variant={variant} {...props}>
        <div
          className="text-xs"
          style={{ color: colors[variant], fill: colors[variant] }}
        >
